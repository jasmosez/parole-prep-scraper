...
class Report {
    constructor() {
        ...
        this.summary = {
            ...
            processingTime: {
                processingSeconds: 0,
                batchCount: 0,
                averageBatchSeconds: 0,
                totalBatchDelaySeconds: 0,
                totalSeconds: 0,
                secondsPerRecord: 0,
                byBatch: []
            },
            ...
            configSettings: {
                ...
                batchDelay: null,
                fewerRecords: null,
                randomizeRecords: null,
                enableTypecast: null,
                enableUpdateRecords: null,
                environment: null
            }
        };
    }
    ...
    setConfigSettings(config) {
        this.summary.configSettings = {
            batchSize: config.airtable.batchSize,
            batchDelay: config.airtable.batchDelay,
            fewerRecords: config.fewerRecords,
            randomizeRecords: config.randomizeRecords,
    ...
    addRecord({recordId, din, outcome, message = '', details = '', changes = []}) {
    ...
    addBatchTime(batchIndex, startTime, endTime) {
        ...
        this.summary.processingTime.processingSeconds += processingSeconds;
        this.summary.processingTime.batchCount++;
        this.summary.processingTime.averageBatchSeconds = 
            this.summary.processingTime.processingSeconds / this.summary.processingTime.batchCount;
        this.summary.processingTime.totalBatchDelaySeconds = (batchIndex - 1) * this.summary.configSettings.batchDelay / 1000;
        this.summary.processingTime.totalSeconds = this.summary.processingTime.processingSeconds + this.summary.processingTime.totalBatchDelaySeconds;
        this.summary.processingTime.secondsPerRecord = this.summary.processingTime.totalSeconds / this.summary.total;
    ...
    getTextReport() {
        ...
        Object.entries(summary.configSettings).forEach(([key, value]) => {
        ...
        const updateFailedRecords = this.getRecordsByOutcome(RecordOutcome.UPDATE_FAILED);
        if (updateFailedRecords.length > 0) {
            report.push('UPDATE_FAILED');
            updateFailedRecords.forEach(record => {
                report.push(`${record.din}: ${record.details}`);
            });
            report.push('');
        }

        // Other outcome sections (excluding CHANGED and UPDATE_FAILED)
        const outcomeTypes = Object.values(RecordOutcome)
            .filter(type => ![RecordOutcome.CHANGED, RecordOutcome.UPDATE_FAILED].includes(type));
